...
module Sidtool
  RSpec.describe FileReader do
    ...
    context 'with empty metadata fields' do
      let(:name) { '' }
      let(:author) { '' }
      let(:released) { '' }

      it 'reads each field as an empty string' do
        sid_file = FileReader.read(@path)
        expect(sid_file.name).to eq('')
        expect(sid_file.author).to eq('')
        expect(sid_file.released).to eq('')
      end
    end

    context 'without a load address' do
      let(:data) { [] }
